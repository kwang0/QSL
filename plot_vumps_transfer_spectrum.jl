...
function usage()
    return """
    Usage:
      julia plot_vumps_transfer_spectrum_fig4a.jl input_file.h5 [output_dir] [max_levels] [label_filter] [first_level]
...
function parse_args(args)
    ...
    return PlotOptions(input_file, output_dir, max_levels, label_filter, first_level)
end
...
function read_run(path::AbstractString)
    h5open(path, "r") do file
        fluxes = read_fluxes(file)
        theta_over_pi = haskey(file, "fluxes_over_pi") ?
            Float64.(vec(real.(read(file, "fluxes_over_pi")))) :
            fluxes ./ pi
        nflux = length(theta_over_pi)

        inverse_xi = Float64.(real.(read_level_matrix(file, "transfer_inverse_xi", nflux)))
        momenta = haskey(file, "transfer_momenta") ?
            Float64.(real.(read_level_matrix(file, "transfer_momenta", nflux))) :
            fill(NaN, size(inverse_xi))
        labels = haskey(file, "transfer_flux_labels") ?
            String.(read_level_matrix(file, "transfer_flux_labels", nflux)) :
        ...
        return (; path, theta_over_pi, inverse_xi, momenta, labels, params)
    end
end
...
function output_stem(path::AbstractString)
    stem = splitext(basename(path))[1]
    return "transfer_spectrum_" * replace(stem, r"[^A-Za-z0-9_.-]" => "_")
end
