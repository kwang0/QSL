...
function triangular_model(C, L, J1, J2, B=0.0, Bperp=0.0, Δ1 = 1.0, Δ2 = 1.0, θ=0.0)
    ...
    for col in range(0,L-1)
        for row in range(0,C-1)
            ...
            os += 0.5*J1*cis(-θ/C), "S+", index, "S-", idx(row + 1, col, C)
            os += 0.5*J1*cis(θ/C), "S-", index, "S+", idx(row + 1, col, C)
            ...
            if (col < L-1)
                ...
                if (row % 2 == 1)
                    ...
                    os += 0.5*J1*cis(-θ/C),  "S+", index, "S-", idx(row + 1, col + 1, C)
                    os += 0.5*J1*cis(θ/C), "S-", index, "S+", idx(row + 1, col + 1, C)
            ...
            os += 0.5*J2*cis(-2*θ/C), "S+", index, "S-", idx(row + 2, col, C)
            os += 0.5*J2*cis(2*θ/C), "S-", index, "S+", idx(row + 2, col, C)

            if ((col < L-1) && (row % 2 == 0))
                os += Δ2*J2, "Sz", index, "Sz", idx(row + 1, col + 1, C)
                os += 0.5*J2*cis(-θ/C), "S+", index, "S-", idx(row + 1, col + 1, C)
                os += 0.5*J2*cis(θ/C), "S-", index, "S+", idx(row + 1, col + 1, C)
            ...
            elseif ((col < L-2) && (row % 2 == 1))
                ...
                os += 0.5*J2*cis(-θ/C), "S+", index, "S-", idx(row + 1, col + 2, C)
                os += 0.5*J2*cis(θ/C), "S-", index, "S+", idx(row + 1, col + 2, C)
...
function main(; C=4, L=6, J1=1.0, J2=0.0, B=0.0, Bperp=0.0, Δ1=1.0, Δ2=1.0, θ=0.0, cutoff=1f-10, maxdim=32)
    N = C * L

    # filename = "/pscratch/sd/k/kwang98/QSL/ground_state_search_YC_C$(C)_L$(L)_J$(J2)_1Delta$(Δ1)_2Delta$(Δ2)_chi$(maxdim)_1f10.h5"
    ...
    filename = "/pscratch/sd/k/kwang98/QSL/ground_state_search_C$(C)_L$(L)_J$(J2)_B$(B)_1Delta$(Δ1)_2Delta$(Δ2)_theta$(θ)_chi$(maxdim).h5"
    sites = siteinds("S=1/2", N; conserve_qns=false)

    n_adiabatic = 5
    θs = LinRange(0, θ, n_adiabatic)

    Hs = MPO[]
    for θ in θs
        push!(Hs, cu(MPO(triangular_model(C, L, J1, J2, B, Bperp, Δ1, Δ2, θ), sites)))
    end
    ...
    for i in 1:nsamples
        ...
        E0 = 1000000
        for θ in θs
            E0, ψ0 = dmrg(Hs[i], ψ0; nsweeps, maxdim, cutoff)
        end
...
main(C=C, L=L, J2=J2, Δ1=Δ, Δ2=Δ, maxdim=maxdim, θ=θ)