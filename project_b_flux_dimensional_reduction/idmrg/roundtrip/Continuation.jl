module RoundTripContinuation
using ProjectBIDMRG, MPSKit, TensorKit, Random, TOML
...
"""Carry the actual endpoint into each successive forward or return point.
An incomplete point is recorded but cannot seed any further point."""
function follow_path(initial,arm;solve,record)
    psi=initial; completed=0
    for j in eachindex(arm["fluxes"])
        result=solve(psi,j,arm["fluxes"][j],arm["iterations"])
        full=length(result.history)==arm["iterations"]
        record(j,result,full)
        full || return (;completed,complete=false,psi=result.psi)
        completed+=1; psi=result.psi
    end
    (;completed,complete=true,psi)
end
...
function run_arm(control_path,name,scratch,compact;deadline=Inf,stop_file="")
    ...
    Random.seed!(r["random_seed"]); B.require_exact_environment()
    ...
    row=Dict("iteration"=>0,"native_error"=>Float64(MPSKit.calc_galerkin(psi,H,psi,envs)),
    ...
    C.write_toml(joinpath(compact,name*"_origin.toml"),Dict("artifact_kind"=>"project_b_roundtrip_origin","schema_version"=>1,
        "control_sha256"=>hash,"parent_sha256"=>r["parent_sha256"],"arm"=>name,"payload"=>origin,
        "history"=>[row],"continuation_accepted"=>false))
    previous=origin; forward=Dict{Int,Any}(0=>origin); points=String[]
    ...
    function solve(x,j,theta,n)
        ...
        function callback(state,env,row,history)
            ...
            payload=R.SP.write_candidate(joinpath(scratch,label*"_iter$i.h5"),state,b,hash,
                Dict("algorithm"=>"VUMPS","theta_over_pi"=>theta),history,"fixed_iteration_roundtrip";kind="checkpoint")
            ...
            push!(checkpoints,cp)
            C.write_toml(joinpath(compact,label*"_iter$i.toml"),merge(cp,Dict("artifact_kind"=>"project_b_roundtrip_checkpoint",
                "control_sha256"=>hash,"parent_sha256"=>r["parent_sha256"],"diagnostic_seed"=>previous,
                "direction"=>arm["directions"][j],"record"=>row,"continuation_accepted"=>false)))
            ...
            println(label," ",arm["directions"][j]," theta/pi=",theta," iteration ",i,"/",n," Galerkin=",row["native_error"]," E=",row["energy_density"]); flush(stdout)
        end
    ...
    function record(j,result,full)
        label=name*"_point$j"
        p=Dict("artifact_kind"=>"project_b_roundtrip_point","schema_version"=>1,"stage"=>label,"arm"=>name,
            "point"=>j,"direction"=>arm["directions"][j],"theta_over_pi"=>arm["fluxes"][j],
            "iteration_budget"=>arm["iterations"],"step_over_pi"=>arm["step_over_pi"],
            "updates_per_theta_over_pi"=>arm["updates_per_theta_over_pi"],"cumulative_updates_before"=>(j-1)*arm["iterations"],
        ...
        pair>=0 && (p["paired_forward"]=forward[pair])
        C.write_toml(joinpath(compact,label*".toml"),p); push!(points,label)
        if full
            previous=checkpoints[end]
            arm["directions"][j]=="forward" && (forward[j]=previous)
        end
    end
    result=follow_path(psi,arm;solve,record)
    C.write_toml(joinpath(compact,name*"_outcome.toml"),Dict("artifact_kind"=>"project_b_roundtrip_outcome","schema_version"=>1,
