...
fixture="$(mktemp -d "$source_root/tmp/relaxation-delivery.XXXXXX")"
cleanup() {
  case "$fixture" in "$source_root"/tmp/relaxation-delivery.*) rm -rf -- "$fixture" ;; *) exit 3 ;; esac
}
trap cleanup EXIT
git init --bare -q "$fixture/origin.git"
git init -q -b codex/relaxation-delivery "$fixture/publisher"
...
git -C "$fixture/publisher" push -q -u origin codex/relaxation-delivery
git clone -q --no-checkout --branch codex/relaxation-delivery "$fixture/origin.git" "$fixture/receiver"
...
git -C "$fixture/publisher" -c commit.gpgsign=false commit -q -m 'Deliver relaxation trial'
...
mapfile -t active < <(sed 's/\r$//' "$pulled/configs/relaxation_continuation_active_control.ref")
[[ ${#active[@]} == 2 && "${active[0]}" == configs/controls/*.toml ]]
control="$pulled/${active[0]}"
[[ "$(sha256sum "$control" | awk '{print $1}')" == "${active[1]}" ]]
"$julia_bin" --startup-file=no "$pulled/scripts/validate_relaxation_continuation.jl" "$control" --code-only
if "$julia_bin" --startup-file=no "$pulled/scripts/validate_relaxation_continuation.jl" "$control" >"$fixture/missing.log" 2>&1; then
  echo 'ERROR: full validation accepted missing tensor data' >&2; exit 1
fi
grep -q 'relaxation input hash mismatch: .*output' "$fixture/missing.log"
[[ ! -e "$pulled/output" ]]
printf '\n# test tampering\n' >>"$pulled/idmrg/src/RelaxationContinuation.jl"
if "$julia_bin" --startup-file=no "$pulled/scripts/validate_relaxation_continuation.jl" "$control" --code-only >"$fixture/tamper.log" 2>&1; then
  echo 'ERROR: modified runtime accepted' >&2; exit 1
fi
grep -q 'relaxation input hash mismatch' "$fixture/tamper.log"
