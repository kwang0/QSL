...
root="$project_root/output/mpskit_solver_pilot_jobs/fullflux"
worker="$project_root/slurm/run_fullflux_continuation_job.sh"
die() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }
require_perlmutter() {
...
latest() {
  ...
  [[ -d "$root" ]] || die 'no fullflux jobs recorded'
  ...
  [[ -n "$directory" ]] || die 'no matching fullflux job ID'
  printf '%s\n' "$directory"
}
action="${1:-}"
case "$action" in
  preflight|plan|submit)
    ...
    if [[ $# == 2 ]]; then control="$2"
      ...
      ref="$project_root/configs/fullflux_continuation_active_control.ref"
      [[ -f "$ref" ]] || die 'missing active fullflux control reference'
      ...
      [[ "$(sha256sum "$control" | awk '{print $1}')" == "${reference[1]}" ]] || die 'active fullflux control hash mismatch'
    fi
    ...
    validation="$("$julia_bin" --startup-file=no "$project_root/scripts/validate_fullflux_continuation.jl" "$control")"
    ...
    if ! bash "$tmp/worker.sh" preflight "$project_root" "$control" "$hash" "$tmp" "$julia_bin"; then
      rm -r "$tmp"; die 'copied worker smoke failed'
    fi
    rm -r "$tmp"
    ...
    printf 'Three independent chi512 VUMPS scans: N=2,4,8; theta/pi=0.15 -> 0.20,0.30,...,1.00.\n'
    printf '126 updates; 30 endpoint/origin analyses. Diagnostic advance continues through native/continuity failures.\n'
    ...
    raw="$(sbatch --parsable --job-name=pb1-fullflux --account="${PHASE1_ACCOUNT:-m4863}" \
      ...
      --output="$run_dir/logs/fullflux-%j.out" --export=ALL \
    ...
    printf 'Submitted three-scan fullflux job %s; run package %s\n' "$id" "$run_dir"
    ;;
  status|progress)
    ...
    [[ "$action" != progress ]] || {
      ...
      [[ ! -f "$run_dir/logs/fullflux-$id.out" ]] || tail -n 40 "$run_dir/logs/fullflux-$id.out"
    }
    ;;
  reconcile) require_perlmutter; pb_reconcile "$project_root" ;;
  analyze)
    run_dir="$(latest)"
    "$julia_bin" --startup-file=no "$project_root/scripts/summarize_fullflux_continuation.jl" "$run_dir"
    ;;
  *) die 'usage: run_fullflux_continuation_cpu.sh preflight|plan|submit [CONTROL] | status|progress|reconcile|analyze; optional RUN_ID=JOB_ID' ;;
esac
