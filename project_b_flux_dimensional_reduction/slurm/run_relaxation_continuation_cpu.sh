...
root="$project_root/output/mpskit_solver_pilot_jobs/relaxation"
worker="$project_root/slurm/run_relaxation_continuation_job.sh"
die() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }
require_perlmutter() {
...
latest() {
  ...
  [[ -d "$root" ]] || die 'no relaxation jobs recorded'
  ...
  [[ -n "$directory" ]] || die 'no matching relaxation job ID'
  printf '%s\n' "$directory"
}
action="${1:-}"
case "$action" in
  preflight|plan|submit)
    ...
    if [[ $# == 2 ]]; then control="$2"
      ...
      ref="$project_root/configs/relaxation_continuation_active_control.ref"
      [[ -f "$ref" ]] || die 'missing active relaxation control reference'
      ...
      [[ "$(sha256sum "$control" | awk '{print $1}')" == "${reference[1]}" ]] || die 'active relaxation control hash mismatch'
    fi
    ...
    validation="$("$julia_bin" --startup-file=no "$project_root/scripts/validate_relaxation_continuation.jl" "$control")"
    ...
    printf 'Fixed chi512 VUMPS: 8/16/32 iterations, step/pi 0.025/0.0125, theta/pi 0.15 to 0.20; baseline 32 and endpoint holds 16.\n'
    printf '464 outer iterations maximum. Every iteration checkpointed to scratch; diagnostic advance only.\n'
    ...
    raw="$(sbatch --parsable --job-name=pb1-relaxation --account="${PHASE1_ACCOUNT:-m4863}" \
      ...
      --output="$run_dir/logs/relaxation-%j.out" --export=ALL \
    ...
    printf 'Submitted relaxation diagnostic %s; run package %s\n' "$id" "$run_dir"
    ;;
  status|progress)
    ...
    [[ "$action" != progress ]] || {
      ...
      [[ ! -f "$run_dir/logs/relaxation-$id.out" ]] || tail -n 40 "$run_dir/logs/relaxation-$id.out"
    }
    ;;
  reconcile) require_perlmutter; pb_reconcile "$project_root" ;;
  analyze)
    run_dir="$(latest)"
    "$julia_bin" --startup-file=no "$project_root/scripts/summarize_relaxation_continuation.jl" "$run_dir"
    ;;
  *) die 'usage: run_relaxation_continuation_cpu.sh preflight|plan|submit [CONTROL] | status|progress|reconcile|analyze; optional RUN_ID=JOB_ID' ;;
esac
